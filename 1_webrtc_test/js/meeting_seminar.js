...
*/
    ...
    pc.ontrack = (e) => {
        ...
        receiveVideos[socketId] = setVideoPosition(role, userName); //새로온 유저의 비디오 만들기
        receiveVideos[socketId].srcObject = e.streams[0];   //새로온 유저의 영상 스트림 설정
    	...
    	pc.onicecandidate = (e) => {
    	    if(e.candidate) {
    	        socket.emit("receiverCandidate", {
    	            candidate: e.candidate,
    	            receiverSocketId: socket.id,
    	            senderSocketId: socketId,   //에러나는 부분
    	        });
    	    }
    	}
    } catch (error) {
