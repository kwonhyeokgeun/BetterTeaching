...
또한 늦게 방에 입장한 사용자는 서버에게 접속 이벤트를 보내고 서버는 이 이벤트를 	받고 해당 클라이언트에게 데이터베이스에 저장되어있던 해당 방의 대화 내용을 전송	해준다. 클라이언트는 모든 대화 내용을 서버로부터 전달받게 되고 이를 채팅창에 추	가하여 사용자가 해당 방에서 이루어졌던 모든 대화를 볼 수 있게된다.
