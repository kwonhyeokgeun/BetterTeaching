...
const socket = io('https://localhost', {secure: true, 'reconnect': true, 'reconnection delay': 500, 'max reconnection attempts': 10});
...
function browserDisconnect() {
    if(roomType == 'meeting'){
        ...
        if(roomType === 'meeting' && socketId === roomLeader && !is_record){  //미팅에서 방장이 나가면 저장여부 물어봄
			...
			is_record = confirm("저장된 녹화영상을 다운로드 받으세요.");
            if(is_record  === true){
...
//function createSenderPeerConnection(stream, purpose, is_audio_true = 1) {
	function createSenderPeerConnection(stream, purpose, is_audio_true) {
