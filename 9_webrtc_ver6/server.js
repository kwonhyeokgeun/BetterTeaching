...
app.post('/login', (request, response) => {
    ...
    if(rooms[requestRoomId]['room_type'] === 'meeting') {
		/*if(numOfUsers[requestRoomId] > 5){ // meeting : 방장 제외 5명
			response.send('<script>alert("정원 초과 입니다");window.location.href="/";</script>');
			return;
		}*/
...
app.post('/make-meeting', (request, response) => {
    ...
    //유효성 체크
    if(roomId.trim().length <= 0 || roomName.trim().length <= 0) {
        response.redirect('/make-meeting');
        return;
    }
...
app.post('/make-seminar', (request, response) => {
    ...
    //유효성 체크
    if(roomId.trim().length <= 0 || roomName.trim().length <= 0) {
        response.redirect('/make-seminar');
        return;
    }
...
io.on('connection', function(socket) {
	...
	console.log("[socket] sender_offer");
        try {
	...
	console.log("[socket] receiver_offer");
        try {
	...
	console.log("[socket] sender_candidate");
        try {
	...
	console.log("[socket] receiver_candidate");
        try {
            if(!message.candidate) return;
	...
	console.log("[socket] meeting_disconnect");
	    ...
	    getMediapipeResult(socketId, roomId); // Mediapipe 결과 도출
	...
	console.log("[socket] seminar_disconnect");
        try{socket.leave(data.roomId);}catch(e){console.error(e);}
        try {
	...
	console.log("[socket] share_disconnect");
	...
	console.log("[socket] request_1_1");
	...
	console.log("[socket] refusal_1_1");
        for(var key in roomList[message.roomId]) {
            if(key === message.target) { // 1:1 건 사람에게
	...
	console.log("[socket] set_room_time");
	...
	console.log("[socket] req_chat");
        //새로 접속한 사용자에게 채팅정보 전송
		try{
			getConnection((conn)=>{
				conn.query("SELECT * FROM chat_db where roomId = '"+data.roomId+"' and roomTime ="+roomToTime[data.roomId], function (err, result, fields) {
					if (err) throw err;
					//console.log(result);
        ...
        /*
        getConnection.query("SELECT * FROM chat_db where roomId = '"+data.roomId+"' and roomTime ="+roomToTime[data.roomId], function (err, result, fields) {
            if (err) throw err;
            //console.log(result);
            console.log('request_chat');
            socket.emit('get_chat',result)
	...
	console.log("[socket] ex");
        console.log("!!XXXX!!")
    });

    socket.on("leader_socket_id_request", (message) => {
	console.log("[socket] leader_socket_id_request");
	...
	console.log("[socket] share_question");
        if(shareSwitch[users[socket.id]['room_id']]) return;
	...
	console.log("[socket] reqUserInfo");
	...
	console.log("[socket] saveCapture");
...
//Mediapipe Pose로 연결
function mediapipe(data, socketId){
    let path = shell.exec('pwd');
    path = path.replace('\n','') + '/captures/' + data.room + '/' + socketId;
    //docker생성 후 접속
    shell.exec('docker run -d -it --name ' + socketId + ' -v ' + path + ':/mediapipe/capture mp:1.0 /bin/bash');
    shell.exec('docker exec -d ' + socketId + ' bazel-bin/mediapipe/examples/desktop/pose_tracking/pose_tracking_out_cpu --calculator_graph_config_file=mediapipe/graphs/pose_tracking/pose_tracking_cpu.pbtxt --input_video_path=capture &');
}

//Mediapipe Pose 결과 도출
function getMediapipeResult(socketId, roomId){
    let path = shell.exec('pwd');
    path = path.replace('\n','') + '/captures/' + roomId + '/' + socketId;
    shell.exec('docker stop ' + socketId);
    shell.exec('docker cp ' + socketId + ':/mediapipe/result.csv ' + path);
    shell.exec('docker rm ' + socketId);
}
